...
function chartTooltipMergesHorizontally(chartEl, currentMeta, currentValue) {
    ...
    let legends = jQuery(".ct-legend li", chartEl);
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    var initChart = function (chartCnt, chartData, undefined) {
        ...
        if (chartData.headerInfo) {
            chartEl.closest(".card-body").prepend(`<div class="card-header-info">jQuery.{chartData.headerInfo}</div>`);
        }
        if (chartData.footerInfo) {
            chartEl.closest(".card-body").append(`<div class="card-footer-info">jQuery.{chartData.footerInfo}</div>`);
        }
        ...
        var availableWidth = jQuery(chartEl).width();
        ...
        if (strokeColor) {
            ...
            chartCtrl.on('draw', function (data) {
                if (data.type === 'bar' && spacing > 0) {
                    ...
                    if (data.element._node.children && jQuery(data.element._node.children[0]).text() !== '') {
                        if (counter === 0) {
                            counter = 1;
                            data.element.attr({
